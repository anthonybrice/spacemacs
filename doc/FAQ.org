...
Emacs and some Spacemacs layers make calls to programs other than Emacs to provide 
some basic functionality. In order for Emacs to find these programs, there are two 
different aspects of Emacs of which you should be aware.

*** The =exec-path= Variable
=exec-path= is a list of directories to search for programs to run in subprocesses.
For example, Emacs usually calls the program =aspell= to provide spell-checking, and 
the default value of =exec-path= is usually enough for Emacs to find that program. However 
if Emacs cannot find a certain program, it is necessary to add the program's directory to 
=exec-path=. To illustrate, to add =npm='s default global-installation directory, place the 
following in your =dotspacemacs/user-config=:

#+BEGIN_SRC emacs-lisp
(add-to-list 'exec-path "/home/<your username>"/.npm-global/bin")
#+END_SRC

To view the value of =exec-path= in your =*Messages*= buffer, type =M-:=, 
then =(print exec-path)=, and =RETURN=.

*** Emacs' =PATH= Environment Variable
In addition to =exec-path=, Emacs defines an environment variable called =PATH= to search for 
programs that are executed within Emacs' shell environment. This is very similar in concept to 
the =PATH= in your OS environment, but note that unless you run start and run Emacs within a 
terminal, Emacs' =PATH= will not reflect the =PATH= defined in your OS environment. To modify Emacs' =PATH=,
use =(setenv "PATH" <some string>)=. To illustrate, to copy the =PATH= from your own user environment into Emacs' 
=PATH=, place the following in your =dotspacemacs/user-config=:

#+BEGIN_SRC emacs-lisp
(setenv "PATH" (shell-command-to-string "echo -n $PATH"))
#+END_SRC

To view the value of Emacs' =PATH= environment variable, type =M-:", then =(getenv "PATH")=, 
and =RETURN=.
